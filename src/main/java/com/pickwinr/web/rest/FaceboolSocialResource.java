...
public class FaceboolSocialResource {
    ...
    @GetMapping("/fb/facebookClientId")
    public ResponseEntity<String> facebookClientId() {
        return ResponseUtil.wrapOrNotFound(Optional.of(facebookClientId));
    }
    ...
    public ResponseEntity<User> profile(@PathVariable String token) {
        ...
        User userProfile = fbClient.fetchObject(
            "me",
            User.class,
    ...
    public ResponseEntity<Connection<Page>> pages(@PathVariable String token) {
        ...
        Connection<Page> userPages = fbClient.fetchConnection(
            "me/accounts",
            Page.class,
